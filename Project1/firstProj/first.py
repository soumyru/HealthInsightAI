...
def login():
    ...
    if request.method=='POST':
        ...
        if user!=None and user[1]==upassword:
            ...
            return render_template("index.html", isLoggedIn=curr_user["isLoggedIn"], email=curr_user["email"])
        else: errormessage='Invalid User.'
    print(errormessage)
...
@app.route('/AboutDisease')
def AboutDisease():    
    print(curr_user["isLoggedIn"])
    return render_template('/AboutDisease.html', message='', isLoggedIn=curr_user["isLoggedIn"], email=curr_user["email"], isModelLoaded=isModelLoaded)
...
def getPromptResponse():
    # Assuming cb.Gemini_Pro_PromptCall_for_chat_title is a function that handles the disease prompt
    # You need to extract the disease name from the request and use it in the function
    disease_name = request.form['prompt']
    result = cb.Gemini_Pro_PromptCall_getDiseaseDetails(disease_name)  # Assuming cb is defined elsewhere
#     result = {
